...
def parse_args():
    ...
    parser.add_argument('-g', '--target',
                        help='Target finish time',
                        required=True)
    parser.add_argument('-l', '--max_length',
                        help='Maximum length of a task',
                        required=True)
    parser.add_argument('-a', '--arrival',
                        help='Include this flag to unconstrain arrival time and bound it at the argument',
                        required=False)
    parser.add_argument('-e', '--min_run',
                        help='Lower bound on running time period lengths. Default value is max_length/100',
                        required=False)
    parser.add_argument('-f', '--max_run',
                        help='Upper bound on running time period lengths. Default value is max_length',
                        required=False)
    parser.add_argument('-m', '--min_block',
                        help='Lower bound on blocking time period lengths. Default value is max_length',
                        required=False)
    parser.add_argument('-n', '--max_block',
                        help='Upper bound on blocking time period lengths. Default value is max_length/100',
                        required=False)
    parser.add_argument('-c', '--scheduled',
                        help='The number of tasks which must be finished by the scheduled model (>=). If both scheduled and unconstrained are not provided, the solver asks the question "can unconstrained finish more tasks than scheduled?"',
                        required=False)
    parser.add_argument('-u', '--unconstrained',
                        help='The number of tasks which must be finished in the unconstrained model (>=). If both scheduled and unconstrained are not provided, the solver asks the question "can unconstrained finish more tasks than scheduled?"',
                        required=False)
    parser.add_argument('-b', '--blocking', nargs='?', const=True, default=False,
                        help='Include this flag to prevent all tasks blocking simultaneously',
                        required=False)
    parser.add_argument('-w', '--work_conserving', nargs='?', const=True, default=False,
                        ...
                        required=False)
    parser.add_argument('-x', '--iff', nargs='?', const=True, default=False,
                        help='Use scheduler iff instead of implication',
                        required=False)
    parser.add_argument('-y', '--geq', nargs='?', const=True, default=False,
                        help='Use >= for the scheduled and unconstrained',
                        required=False)
...
#target = 13  #At least 12 for 5 tasks, 2 steps
...
geq = args.geq
min_run = float(args.min_run)
if not min_run:
    min_run = max_length/100.0
max_run = float(args.max_run)
if not max_run:
    max_run = max_length
min_block = float(args.min_block)
if not min_block:
    min_block = max_length/100.0
max_block = float(args.max_block)
if not max_block:
    max_block = max_length

scheduled = 0
unconstrained = 0
fixed_values = True
if (not args.unconstrained) and (not args.scheduled):
    fixed_values = False
elif args.unconstrained and args.scheduled:
    scheduled = int(args.scheduled)
    unconstrained = int(args.unconstrained)
else:
    print("Usage: Either both or neither of command line arguments --scheduled (-c) and --unconstrained (-u) must be provided. Exiting ...")
    exit()
...
for i in range(0,tasks):
    for j in range(0,steps):
        ...
        sol.add(rf1[i][j] - rs1[i][j] >= min_run)
        sol.add(rf2[i][j] - rs2[i][j] >= min_run)
...
for i in range(0,tasks):
    for j in range(0,steps):
        ...
        if blocking:
            #Not all tasks blocking at the same time
            #For each blocking period, there exists a task (or over tasks) which does not block during this period (and over steps)
            #It's ok if l = i, since the same task will always overlap
            sol.add(Or([And([Or(bf1[l][k] <= bs1[i][j],bf1[i][j] <= bs1[l][k]) for k in range(0,steps)]) for l in range(0,tasks)]))
            sol.add(Or([And([Or(bf2[l][k] <= bs2[i][j],bf2[i][j] <= bs2[l][k]) for k in range(0,steps)]) for l in range(0,tasks)]))

if fixed_values:
    if geq:
        # Number of tasks that must be finished by scheduler
        sol.add(scheduled <= Sum([If(bf1[i][steps-1] <= target,1,0) for i in range(0,tasks)]))
        # Number of tasks that must be finished by unconstrained
        sol.add(unconstrained <= Sum([If(bf2[i][steps-1] <= target,1,0) for i in range(0,tasks)]))
    else:
        # Number of tasks that must be finished by scheduler
        #sol.add(scheduled == Sum([If(bf1[i][steps-1] <= target,1,0) for i in range(0,tasks)]))
    ...
    sol.add(Sum([If(bf1[i][steps-1] <= target,1,0) for i in range(0,tasks)]) < Sum([If(bf2[i][steps-1] <= target,1,0) for i in range(0,tasks)]))

temp_list = [tasks, steps, target, max_length, arrival if arrival else 0, min_run, max_run, min_block, max_block, scheduled if scheduled else 0, unconstrained if unconstrained else 0, blocking, work_conserving, iff, geq]
...
output_list = [tasks, steps, target, max_length, arrival if arrival else 0, min_run, max_run, min_block, max_block, scheduled if scheduled else 0, unconstrained if unconstrained else 0, blocking, work_conserving, iff, geq, solved, stop-start]
