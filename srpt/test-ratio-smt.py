...
def parse_args():
    ...
    parser.add_argument('-g', '--goal',
                        help='Deadline to finish tasks. Default value is steps * tasks * epsilon.',
                        required=False)             
...
beta = float(args.beta) if args.beta else 1.0

geq = args.geq
min_block = alpha * epsilon if alpha else None
max_block = beta * epsilon

target = float(args.goal) if args.goal else steps * tasks * epsilon
print('Deadline = '+str(target))



scheduled = int(args.scheduled) if args.scheduled else None
unconstrained = int(args.unconstrained) if args.unconstrained else None
...
temp_list = [tasks, steps, epsilon, alpha, beta, scheduled if scheduled else 0, unconstrained if unconstrained else 0, work_conserving, geq]
